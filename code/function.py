...
def Model_synthetic_spo(c_01,X_train, y_train, X_test, X_valid, y_valid):
    reset_random_seeds()
    ...
    model.compile(
      optimizer=tf.keras.optimizers.Adam(),
      loss=spo_plus_loss(c_01)
    )
    
    model.fit(X_train,y_train, epochs=30, validation_data = (X_valid,y_valid) ,shuffle = True, batch_size=16 , verbose =0)
...
def Model_credit_spo(c_01,X_train, y_train, X_test, X_valid, y_valid):
    reset_random_seeds()
...
def Model_churn_spo(c_01,X_train, y_train, X_test, X_valid, y_valid):
    reset_random_seeds()
...
def algorithm_spo(c_01, dataset, N1_ratio,N2_ratio):
    p=1
    list_p = []
    list_cost = []
    list_num1 = []
    ...
    if dataset == 'synthetic': 
        ...
        y_train_pred, y_test_pred = Model_synthetic_spo(c_01,X_train, y_train, X_test, X_valid, y_valid)

    elif dataset == 'churn': 
        X_train, y_train, X_test, y_test, X_valid, y_valid= fu.churn()
        y_train_pred, y_test_pred = Model_churn_spo(c_01,X_train, y_train, X_test, X_valid, y_valid)

    elif dataset == 'credit': 
        X_train, y_train, X_test, y_test, X_valid, y_valid= fu.credit()
        y_train_pred, y_test_pred = Model_credit_spo(c_01,X_train, y_train, X_test, X_valid, y_valid)
    ...
    fpr, fnr, thresholds = fu.fpr_fnr(y_train_pred, y_train, y_test)

    interfpr_con, interfnr_con, threshold_con = fu.find_cost_intersection_within(c_01,y_train, y_test, fpr, fnr, thresholds,N1,N2)
    ...
    precision,recall,f1, auc,mcc, gmean, con_mat = fu.metrics(y_test, y_test_classification)
    ...
    # saving the dataframe 
    df.to_csv('result/{}/spo/{}_{}_{}.csv'.format(location,method,N1_ratio,N2_ratio))
